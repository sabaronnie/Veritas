...
Final Approach: API-based fact-checking using OpenAI GPT-4
...
Old Architecture Codes: https://colab.research.google.com/drive/1QGZW5_RWmaWQ9PTqlHhRW3h0Nosb2UIT#scrollTo=0TMOLn8iONkx&uniqifier=2 
...
_Importance of MongoDB:_
-	limited to last week and updates twice a day to preserve accuracy
-	avoids the need to demand scrape all similar articles, augmenting time efficiency

We initially tried to avoid using a database, and we looked into some technologies that help with on-demand scraping. However, we decided to stick with the database to avoid the unpredictable nature of live scraping. Sometimes some websites can unpredictably rate-limit our data requests which ends up severly delaying the pipeline. We experienced that first-hand while initially exploring and scraping from different sources, where we were forced to include a time delay between requests, and even when we did not, we faced significant time constraints for large amounts of data.
Also, our stored data only comprises a week's worth of data, meaning our storage requirements will remain relatively limited to that 1-week window.
When users submit URLs, the system  performs fast comparisons against similar articles using our pre-scraped database based on a +-1 day window to ensure temporal relevance while delivering results in just 3-8 seconds. 
This approach maintains comprehensive coverage of the Lebanese news landscape while providing instant fact-checking through intelligent time-based filtering and optimized database queries. It also allows us to maintain a predictable and reasonable response rate of the payload.
...
## Phase 3: AI Analysis
-	GPT-4 processes structured JSON data containing both the user's article and similar comparison articles
