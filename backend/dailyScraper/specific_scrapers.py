...
def scrape_mtv(MAX_AGE, MTV_API, PAGE_SIZE=1):
    ...
    article = []
    while True:
        api_url = MTV_API.format(start, end)
        print(f"[API] Fetching {start} → {end}")

        try:
            r = requests.get(api_url, headers=HEADERS, timeout=10)
        except:
            print("[X] API connection failed — stopping.")
            break

        if r.status_code != 200:
            print("[X] HTTP error", r.status_code, "— stopping.")
            break

        data = r.json()
        if not data:
            print("[!] No more articles returned — finished.")
            break
        ...
        for item in data:
            ...
            # Extract text from API (HTML → clean text)
            raw_html = item.get("Text")
            clean_body = clean_text(raw_html)
